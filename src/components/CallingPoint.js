...
const CallingPoint = ({
  ...
  <div className={`CallingPoint${hasDeparted ? ' -departed' : ''}`}>
    <time className="CallingPoint__scheduled-at">
      {scheduledAt}
    </time>
    <div
    ...
    >
      {isTrainHere &&
        <div
          className={`CallingPoint__train${hasDeparted ? ' -departed' : ''}`}
        />}
    </div>
    <dl className="CallingPoint__status">
      <dt className="CallingPoint__station">
        {station}
      </dt>
      <dd className="CallingPoint__estimated-at">{`${hasDeparted
        ? 'Dept.'
        : 'Exp.'} ${estimatedAt}`}</dd>
    </dl>
  </div>;
